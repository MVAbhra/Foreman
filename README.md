...
This README was generated by inspecting the repository's actual code (`pom.xml`, controllers, `application.properties`, `Dockerfile`) rather than pre-existing documentation, since the repo did not contain a README. A few things worth flagging for whoever maintains this:

- There's no license file, CI config, or migration tooling (e.g. Flyway/Liquibase) — schema changes rely on Hibernate `ddl-auto=update`, which is fine for development but risky for production.
- The notification microservice client exists (`NotificationClient`) but that service itself isn't in this repo — it needs to be run separately for notification-dependent features to work.
